...
class JigsawDataOriginal(JigsawData):
    ...
    def __init__(
        ...
        super().__init__(
            train_csv_file=train_csv_file, test_csv_file=test_csv_file, train=train, add_test_labels=add_test_labels
        )
...
class JigsawDataBias(JigsawData):
    ...
    def __getitem__(self, index):
        ...
        target_dict = self.filter_entry_labels(
            entry, self.classes + self.identity_classes, threshold=0.5, soft_labels=self.soft_labels
        )
